  ...
  return (
    ...
    >
      ...
      >
        <div className={isProjectJourney ? "w-full h-full" : "max-w-7xl mx-auto px-4 sm:px-10 pb-48"}>
          {isEnquiry ? (
            ...
            <div className={`transition-opacity duration-1000 ${showGalleryItems ? "opacity-100" : "opacity-0"}`}>
              ...
              {!isProjectJourney && !isHomeInsight && (
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-24 px-4 sm:px-10 mt-32">
        ...
        {/* Sitewide footer — placed inside the scrollable container so it
            appears at the true bottom of every section's content, after
            scrolling down. The section root above is `fixed inset-0`, so a
            Footer rendered outside this scroll container would not behave
            like a normal end-of-page footer. */}
        <Footer />
      </div>
    </div>
  );
